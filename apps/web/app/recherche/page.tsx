...
export default async function SearchPage({searchParams}:{searchParams:SearchParams}) {
  ...
  return <div className={styles.page}><SiteHeader/><main className={styles.shell}>
    ...
    <div className={styles.layout}><CatalogFilterPanel/><section className={styles.results}><div className={styles.toolbar}><div><b>{data.total.toLocaleString("fr-FR")} annonce{data.total>1?"s":""}</b><span> trouvée{data.total>1?"s":""}</span></div></div>
      ...
      {!data.items.length?<div className={styles.empty}>Aucune annonce ne correspond à votre recherche pour le moment.</div>:<div className={styles.grid}>{data.items.map(item=>{
        const href=item.slug?`/annonce/${item.slug}`:"#";
        return <article className={styles.card} key={item.id}><div className={styles.image}><a className={styles.imageLink} href={href} aria-label={item.title??item.category.name}>{item.imageUrl?<img src={item.imageUrl} alt={item.title??item.category.name}/>:<span>📦</span>}</a><div className={styles.favoriteSlot}><FavoriteButton listingId={item.id} compact/></div>{item.sellerReputation.trust.reliableSeller&&<em className={styles.reliable}>✓ Vendeur fiable</em>}</div><div className={styles.cardBody}><a className={styles.cardMainLink} href={href}><div className={styles.cardTop}><h2>{item.title??item.category.name}</h2></div><strong>{money(item.priceMinor,item.currency)}</strong><p>{meta(item)}</p><div className={styles.reputation}>{item.sellerReputation.reviewAverage!=null?<span><b>★ {item.sellerReputation.reviewAverage.toFixed(1)}</b> · {item.sellerReputation.reviewCount} avis</span>:<span>Nouveau vendeur</span>}<small>Confiance {item.sellerReputation.trust.score}/100</small></div><footer><span>📍 {item.city??"France"}</span><small>{age(item.publishedAt??item.createdAt)}</small></footer></a></div></article>})}</div>}
    </section></div>
  </main></div>;
}
