...
export default function DsaNoticePage() {
  ...
  useEffect(()=>{const q=new URLSearchParams(window.location.search);setPrefill({targetType:q.get("type")||"LISTING",targetId:q.get("id")||"",contentUrl:q.get("url")||""})},[]);
  ...
  return <><main className="shell" style={{ paddingBlock: 56, maxWidth: 820 }}>
    ...
    <section style={{marginTop:36,padding:22,border:"1px solid #e5e4ed",borderRadius:18,background:"#fafafe"}}>
      ...
      {lookupResult&&<div style={{marginTop:16,padding:16,borderRadius:14,background:"#fff",border:"1px solid #e8e7ef"}}><strong>Statut : {({RECEIVED:"Reçu",UNDER_REVIEW:"En cours d’examen",ACTIONED:"Mesure prise",REJECTED:"Signalement rejeté",RESOLVED:"Traité"} as Record<string,string>)[lookupResult.status]??lookupResult.status}</strong>{lookupResult.decisionReason&&<p style={{marginBottom:0,color:"#59586a"}}>{lookupResult.decisionReason}</p>}</div>}
    </section>
  </main></>;
}
