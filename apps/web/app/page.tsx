...
type Listing = { title: string; price: string; city: string; meta: string; tone: string };
...
const categoryRows: { title: string; slug: string; items: Listing[] }[] = [
...
export default function HomePage() {
  return (
    <div id="top" className="page-shell">
      <SiteHeader />
      <main>
        <section className="mobile-ai-search" aria-label="Recherche assistée"><div className="mobile-ai-search-shell"><div className="mobile-ai-search-head"><span className="mobile-ai-badge">IA</span><strong>Que cherchez-vous ?</strong></div><form action="/recherche" method="get" role="search"><input name="q" type="search" aria-label="Décrivez ce que vous recherchez" placeholder="Ex. iPhone récent à Lyon, moins de 700 €" /><button type="submit" aria-label="Rechercher">→</button></form><small>Décrivez simplement votre besoin, nous lançons la recherche correspondante.</small></div></section>
        <section className="hero-section"><div className="shell hero-layout"><div className="hero-copy"><div className="hero-kicker"><span className="status-dot" /> La marketplace française nouvelle génération</div><h1>Tout ce que vous cherchez.<br /><span>Juste à côté.</span></h1><p className="hero-lead">Achetez, vendez et découvrez des milliers d'annonces partout en France avec une expérience simple, rapide et sécurisée.</p><form className="search-panel" role="search" action="/recherche" method="get"><label className="search-field search-field-main"><span className="field-icon" aria-hidden="true">⌕</span><span className="sr-only">Que recherchez-vous ?</span><input name="q" type="search" placeholder="Que recherchez-vous ?" /></label><label className="search-field search-field-location"><span className="field-icon" aria-hidden="true">⌖</span><span className="sr-only">Localisation</span><input name="city" type="text" placeholder="Toute la France" /></label><button className="button button-primary search-button" type="submit">Rechercher</button></form><div className="quick-links" aria-label="Recherches populaires"><span>Populaire :</span><a href="/recherche?q=iPhone">iPhone</a><a href="/recherche?q=Peugeot%203008">Peugeot 3008</a><a href="/recherche?q=Appartement">Appartement</a><a href="/recherche?q=V%C3%A9lo%20%C3%A9lectrique">Vélo électrique</a></div></div><div className="hero-showcase" aria-label="Aperçu de la plateforme"><div className="showcase-glow" /><article className="showcase-card showcase-main-card"><div className="showcase-photo phone-art"><span className="phone-camera camera-one" /><span className="phone-camera camera-two" /><span className="phone-camera camera-three" /><span className="phone-logo">●</span></div><div className="showcase-card-body"><div><span className="listing-badge">Achat sécurisé</span><h2>iPhone 16 Pro · 256 Go</h2><p>Lyon 2e · Comme neuf</p></div><strong>829 €</strong></div></article><div className="floating-card floating-offer"><span className="floating-icon">€</span><div><small>Nouvelle offre</small><strong>780 €</strong></div><span className="success-pill">+12%</span></div><div className="floating-card floating-trust"><span className="floating-icon verified">✓</span><div><small>Profil</small><strong>Vérifié</strong></div></div></div></div></section>
