...
function MobileMarketplaceCard({listing}:{listing:Listing}){
  ...
  return <article className={`mobile-market-card${promoType?` mobile-market-card-${promoType}`:""}`}>
    <div className="mobile-market-card-photo">
      <a href={href} aria-label={listing.title??listing.category.name}>{listing.imageUrl?<Image src={listing.imageUrl} alt={listing.title??"Annonce Petit Annonces"} fill sizes="(max-width:720px) 50vw, 25vw" quality={68}/>:<ListingCategoryPlaceholder category={listing.category}/>}</a>
...
function MarketplaceCard({listing,spotlight=null}:{listing:Listing;spotlight?:"URGENT"|"FEATURED"|null}){
  ...
  return <article className={`market-home-card${promoted?" market-home-card-promoted":""}${spotlight?` market-home-card-${spotlight.toLowerCase()}`:""}`}>
    <div className="market-home-card-image">
      <a href={href} aria-label={listing.title??listing.category.name}>{listing.imageUrl?<Image src={listing.imageUrl} alt={listing.title??"Annonce Petit Annonces"} fill sizes="(max-width: 700px) 46vw, 20vw" quality={68}/>:<ListingCategoryPlaceholder category={listing.category}/>}</a>
  ...
  return (
    <div id="top" className="page-shell home-page-shell">
      ...
      <main className="home-main-order">
        ...
        <nav className="mobile-legacy-home mobile-home-mini-footer" aria-label="Liens utiles"><a href="/assistance">Aide</a><a href="/conformite">Sécurité</a><a href="/conditions-generales">CGU</a></nav>
