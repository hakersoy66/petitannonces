...
export default function FavoritesPage(){
 ...
 return <div className={styles.page}><main className={styles.shell}><AccountSidebar/><section className={styles.content}>
  <header className={styles.hero}><div><span className={styles.eyebrow}>Votre sélection</span><h1>Mes favoris</h1><p>Gardez vos annonces préférées sous la main, comparez-les et reprenez votre recherche quand vous le souhaitez.</p></div><div className={styles.heroSide}><strong>{items.length}</strong><span>annonce{items.length>1?"s":""} enregistrée{items.length>1?"s":""}</span><a href="/recherche"><AppIcon name="search"/> Continuer ma recherche</a></div></header>
