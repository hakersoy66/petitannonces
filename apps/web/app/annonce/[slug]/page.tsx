...
export default async function ListingPage({params}:Props){
 ...
 const safety=listing.productSafety;
 const hasSafety=Boolean(safety&&(safety.manufacturerName||safety.productIdentifier||safety.model||safety.ean||safety.safetyWarning||safety.responsiblePersonName));
 const productSafetyCard=hasSafety?<div style={{borderTop:"1px solid #ececf1",padding:"18px 20px 20px"}}><h2 style={{margin:"0 0 12px",fontSize:17}}>Sécurité & traçabilité du produit</h2><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:10,fontSize:12,lineHeight:1.5,color:"#4c4a58"}}>{safety?.manufacturerName&&<div><strong>Fabricant</strong><br/>{safety.manufacturerName}{safety.manufacturerPostalAddress&&<><br/>{safety.manufacturerPostalAddress}</>}{safety.manufacturerEmail&&<><br/><a href={`mailto:${safety.manufacturerEmail}`}>{safety.manufacturerEmail}</a></>}</div>}{(safety?.productIdentifier||safety?.model||safety?.ean)&&<div><strong>Identification</strong><br/>{[safety.productIdentifier,safety.model&&`Modèle ${safety.model}`,safety.ean&&`EAN ${safety.ean}`].filter(Boolean).join(" · ")}</div>}{safety?.responsiblePersonName&&<div><strong>Personne responsable dans l’UE</strong><br/>{safety.responsiblePersonName}{safety.responsiblePersonPostalAddress&&<><br/>{safety.responsiblePersonPostalAddress}</>}{safety.responsiblePersonEmail&&<><br/><a href={`mailto:${safety.responsiblePersonEmail}`}>{safety.responsiblePersonEmail}</a></>}</div>}{safety?.ceMarked===true&&<div><strong>Conformité</strong><br/>Marquage CE déclaré par le vendeur lorsque applicable.</div>}</div>{safety?.safetyWarning&&<div style={{marginTop:12,padding:12,borderRadius:12,background:"#fff6e9",border:"1px solid #f1dfbd",fontSize:12,lineHeight:1.55}}><strong>Avertissements de sécurité</strong><p style={{margin:"5px 0 0",whiteSpace:"pre-line"}}>{safety.safetyWarning}</p></div>}</div>:null;
