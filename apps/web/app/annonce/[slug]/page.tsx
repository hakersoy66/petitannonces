...
function stars(value:number|null){if(value==null)return "Aucune évaluation";const rounded=Math.round(value);return `${"★".repeat(rounded)}${"☆".repeat(5-rounded)} ${value.toFixed(1)}/5`;}
...
export default async function ListingPage({ params }: Props) {
  ...
  return <div className={styles.page}><SiteHeader/><main className={styles.shell}>
    ...
    <div className={styles.layout}><section className={styles.main}>
      ...
      <div className={styles.topline}><div className={styles.titleBlock}><h1>{title}</h1><div className={styles.meta}><span>📍 {location}</span>{listing.publishedAt&&<span>🕒 {new Intl.DateTimeFormat("fr-FR").format(new Date(listing.publishedAt))}</span>}</div></div><div className={styles.actions}><FavoriteButton listingId={listing.id} compact/><button className={styles.iconBtn} aria-label="Partager l’annonce">↗</button></div></div>
