...
export function CookieConsent() {
  ...
  if (!open) return <button type="button" className="cookie-manage-button" onClick={()=>{setSettings(true);setOpen(true)}} style={{position:"fixed",right:16,bottom:compact?"calc(72px + env(safe-area-inset-bottom, 0px))":16,zIndex:69,border:"1px solid #d9d7e5",background:"#fff",color:"#36344a",borderRadius:999,padding:compact?"8px 11px":"10px 14px",fontWeight:800,fontSize:compact?10:12,boxShadow:"0 8px 28px rgba(24,22,55,.12)",cursor:"pointer"}}>Gérer les cookies</button>;
