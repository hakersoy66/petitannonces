...
function DynamicField({ filter, categorySlug }: { filter: CatalogFilter; categorySlug: string }) {
  ...
  if (filter.type === "MULTISELECT") {
    return (
      <fieldset className={styles.multiField}>
        ...
        <div>{filter.options?.map((option) => <label key={option}><input type="checkbox" /> {option}</label>)}</div>
      </fieldset>
    );
  }
    ...
    <div className={styles.browser}>
      ...
      <div className={styles.content}>
        <div>
          ...
          <div className={styles.children}>
            {root?.children?.map((category) => (
              <button type="button" key={category.slug} onClick={() => setChildSlug(category.slug)} className={category.slug === child?.slug ? styles.activeChild : styles.child}>
        ...
        <aside className={styles.preview}>
          <p className={styles.kicker}>Formulaire dynamique</p>
          ...
          <div className={styles.dynamicForm}>
            {(child?.filters ?? []).map((filter) => <DynamicField key={filter.key} filter={filter} categorySlug={child?.slug ?? ""} />)}
          </div>

          {child?.domain === "REAL_ESTATE" && (
            <fieldset className={styles.multiField}>
              <legend>Équipements & prestations</legend>
              <div>{propertyExtras.map((item) => <label key={item}><input type="checkbox" /> {item}</label>)}</div>
            </fieldset>
          )}
          ...
          {child?.domain === "ANIMAL" && <div className={styles.notice}>Identification et informations réglementaires adaptées à l’animal.</div>}
        </aside>
      </div>
    </div>
  );
