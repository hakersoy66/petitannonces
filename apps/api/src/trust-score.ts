...
export function calculateTrustScore(input: TrustScoreInput): TrustScore {
  ...
  const verificationPoints = input.verified ? 15 : 0;
  const emailVerificationPoints = input.emailVerified ? 5 : 0;
  ...
  const score = Math.round(Math.max(0, Math.min(100, verificationPoints + emailVerificationPoints + salesPoints + agePoints + reviewPoints)));
