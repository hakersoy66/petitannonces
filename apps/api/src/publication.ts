...
export async function registerPublicationRoutes(app: FastifyInstance) {
  ...
  app.get("/listings/:id/publication-check", async (request, reply) => {
    ...
    const [safetyRows,disclosureRows]=await Promise.all([
      prisma.$queryRawUnsafe<Array<{manufacturerName:string|null;productIdentifier:string|null;model:string|null;ean:string|null}>>(`SELECT "manufacturerName","productIdentifier","model","ean" FROM "ListingProductSafety" WHERE "listingId"=$1 OR "listingId"=$2 ORDER BY CASE WHEN "listingId"=$1 THEN 0 ELSE 1 END LIMIT 1`,params.data.id,fallbackComplianceId).catch(()=>[]),
