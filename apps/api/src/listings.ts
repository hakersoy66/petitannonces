...
async function ownedDraft(id: string, userId: string) {
  return prisma.listing.findFirst({ where: { id, sellerId: userId, status: "DRAFT" }, include: { category: true } });
}
...
export async function registerListingRoutes(app: FastifyInstance) {
  ...
  app.get("/categories/:slug/attributes", async (request, reply) => {
    ...
    return category ? reply.send({ category }) : reply.code(404).send({ error: "category_not_found" });
  });
  ...
  app.post("/listings/drafts", async (request, reply) => {
    ...
    const listing = await prisma.listing.create({ data: { sellerId: user.id, categoryId: category.id }, include: { category: true } });
  ...
  app.patch("/listings/:id/basics", async (request, reply) => {
    ...
    return reply.send({ listing: await prisma.listing.update({ where: { id: draft.id }, data: body.data }) });
  });
  ...
  app.put("/listings/:id/attributes", async (request, reply) => {
    ...
    if (!params.success || !body.success) return reply.code(400).send({ error: "invalid_request" });
    const draft = await ownedDraft(params.data.id, user.id); if (!draft) return reply.code(404).send({ error: "draft_not_found" });
    const definitions = await prisma.categoryAttribute.findMany({ where: { categoryId: draft.categoryId, id: { in: body.data.values.map(v => v.attributeId) } }, include: { options: true } });
  ...
  app.post("/listings/:id/vehicle/from-plate", async (request, reply) => {
    ...
    try {
      ...
      const data = {
        registrationPlateHash: plateHash(normalized), registrationPlateLast4: normalized.replace(/-/g, "").slice(-4), dataSource: v.source, dataVerifiedAt: new Date(),
      ...
      const vehicle = await prisma.vehicleDetails.upsert({ where: { listingId: draft.id }, create: { listingId: draft.id, ...data }, update: data });
      return reply.send({ vehicle, plateStored: false });
    } catch (error) {
  ...
  app.post("/listings/:id/submit", async (request, reply) => {
    ...
    for (const a of listing.category.attributes) if (!setIds.has(a.id)) errors.push(`required_attribute:${a.key}`);
