...
export async function registerAuthRecoveryRoutes(app: FastifyInstance) {
  app.post("/auth/forgot-password", async (request, reply) => {
    ...
    const user=await prisma.user.findUnique({where:{email:body.data.email}});if(!user)return reply.send({requested:true});
    const token=newOpaqueToken();
    await prisma.passwordResetToken.updateMany({where:{userId:user.id,usedAt:null},data:{usedAt:new Date()}});
    await prisma.passwordResetToken.create({data:{userId:user.id,tokenHash:sha256(token),expiresAt:new Date(Date.now()+RESET_TTL_MS)}});
    await queueTransactionalEmail({userId:user.id,eventKind:"AUTH_PASSWORD_RESET",title:"Réinitialisez votre mot de passe",body:"Vous avez demandé la réinitialisation de votre mot de passe Petit Annonces. Ce lien est valable 30 minutes. Si vous n’êtes pas à l’origine de cette demande, ignorez cet e-mail.",actionUrl:passwordResetActionUrl(token),metadata:{purpose:"PASSWORD_RESET"}});
    return reply.send({requested:true,...(process.env.NODE_ENV!=="production"?{devResetToken:token}:{})});
  });

  app.post("/auth/reset-password", async (request, reply) => {
    const body=z.object({token:z.string().min(20),password:z.string().min(10).max(128)}).safeParse(request.body);if(!body.success)return reply.code(400).send({error:"invalid_request"});
    const record=await prisma.passwordResetToken.findUnique({where:{tokenHash:sha256(body.data.token)}});if(!record||record.usedAt||record.expiresAt<=new Date())return reply.code(400).send({error:"invalid_or_expired_token"});
