...
function canSendMessage(userId: string) {
  ...
  entries.push(now);
  recentMessages.set(userId, entries);
  return true;
}

function looksUnsafeMessage(body: string) {
  const normalized = body.toLowerCase();
  const suspicious = ["western union", "mandat cash", "crypto uniquement", "paiement hors plateforme", "telegram uniquement"];
  return suspicious.some((term) => normalized.includes(term));
}
...
export async function registerMessagingRoutes(app: FastifyInstance) {
  app.post("/conversations", async (request, reply) => {
    ...
    if (!user) return;
    const parsed = z.object({ listingId: z.string().min(1) }).safeParse(request.body);
    ...
    const conversation = await prisma.conversation.upsert({
      where: { listingId_buyerId_sellerId: { listingId: listing.id, buyerId: user.id, sellerId: listing.sellerId } },
  ...
  app.get("/conversations", async (request, reply) => {
    ...
    const conversations = await prisma.conversation.findMany({
      ...
      include: {
        listing: { select: { id: true, title: true, slug: true, priceMinor: true, currency: true } },
  ...
  app.get("/conversations/:id/messages", async (request, reply) => {
    ...
    const conversation = await getConversationForUser(parsed.data.id, user.id);
  ...
  app.post("/conversations/:id/messages", async (request, reply) => {
    ...
    const conversation = await getConversationForUser(params.data.id, user.id);
    if (!conversation || conversation.status !== "OPEN") return reply.code(404).send({ error: "conversation_not_available" });
    ...
    const message = await prisma.message.create({
      data: {
        ...
        senderId: user.id,
        kind: body.data.attachmentUrl ? "FILE" : "TEXT",
        body: body.data.body,
        attachmentUrl: body.data.attachmentUrl,
        attachmentName: body.data.attachmentName,
        attachmentMime: body.data.attachmentMime,
        attachmentSize: body.data.attachmentSize,
      },
    });
  ...
  app.post("/conversations/:id/offers", async (request, reply) => {
    ...
    if (!user) return;
    const params = z.object({ id: z.string().min(1) }).safeParse(request.params);
    const body = z.object({ amount: z.number().positive(), parentOfferId: z.string().optional() }).safeParse(request.body);
    if (!params.success || !body.success) return reply.code(400).send({ error: "invalid_offer" });
    const conversation = await getConversationForUser(params.data.id, user.id);
    if (!conversation || conversation.status !== "OPEN") return reply.code(404).send({ error: "conversation_not_available" });
    const amountMinor = Math.round(body.data.amount * 100);
    if (amountMinor < OFFER_MIN_MINOR) return reply.code(400).send({ error: "offer_too_low" });
    ...
    if (body.data.parentOfferId) {
      const parent = await prisma.offer.findFirst({ where: { id: body.data.parentOfferId, conversationId: conversation.id } });
      if (!parent || parent.recipientId !== user.id || parent.status !== "PENDING") return reply.code(409).send({ error: "offer_cannot_be_countered" });
      parentOfferId = parent.id;
      await prisma.offer.update({ where: { id: parent.id }, data: { status: "COUNTERED", respondedAt: new Date() } });
    }
    const result = await prisma.$transaction(async (tx) => {
      const message = await tx.message.create({ data: { conversationId: conversation.id, senderId: user.id, kind: "OFFER", body: `Offre: ${(amountMinor / 100).toFixed(2)} €` } });
  ...
  app.post("/offers/:id/respond", async (request, reply) => {
    ...
    if (!user) return;
    const params = z.object({ id: z.string().min(1) }).safeParse(request.params);
    const body = z.object({ action: z.enum(["ACCEPT", "DECLINE"]) }).safeParse(request.body);
    if (!params.success || !body.success) return reply.code(400).send({ error: "invalid_offer_response" });
    const offer = await prisma.offer.findUnique({ where: { id: params.data.id } });
    if (!offer || offer.recipientId !== user.id) return reply.code(404).send({ error: "offer_not_found" });
    ...
    await prisma.offer.update({ where: { id: offer.id }, data: { status, respondedAt: new Date() } });
    if (status === "ACCEPTED") {
      await prisma.offer.updateMany({
        where: { conversationId: offer.conversationId, id: { not: offer.id }, status: "PENDING" },
  ...
  app.post("/offers/:id/withdraw", async (request, reply) => {
    ...
    if (!user) return;
    const parsed = z.object({ id: z.string().min(1) }).safeParse(request.params);
    if (!parsed.success) return reply.code(400).send({ error: "invalid_offer" });
    const offer = await prisma.offer.findUnique({ where: { id: parsed.data.id } });
    if (!offer || offer.makerId !== user.id) return reply.code(404).send({ error: "offer_not_found" });
    if (offer.status !== "PENDING") return reply.code(409).send({ error: "offer_already_resolved" });
    await prisma.offer.update({ where: { id: offer.id }, data: { status: "WITHDRAWN", respondedAt: new Date() } });
    return reply.send({ offerId: offer.id, status: "WITHDRAWN" });
  });
}
