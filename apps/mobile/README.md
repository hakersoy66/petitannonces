...
## iOS release without EAS

The repository includes `.github/workflows/ios-native-testflight.yml` for an EAS-independent iOS release path. It runs on a GitHub-hosted macOS runner, generates the native iOS project with Expo prebuild, installs CocoaPods, and can either run an unsigned native smoke build or produce a signed IPA and upload it to TestFlight with Fastlane.
